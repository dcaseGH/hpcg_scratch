...
int ComputeSPMV_ref( const SparseMatrix & A, Vector & x, Vector & y) {
  ...
  assert(y.localLength>=A.localNumberOfRows);
    ...
    ExchangeHalo(A,x);
#endif
  std::cout << "Doing 27pt stencil " << x.localLength << " " << " " << y.localLength << std::endl;
...
//  for (int i = 0; i< y.localLength ; i++) yv[i] = 0.0;
  ...
  //std::cout << "Local implementation at first (no mpi) "<< nx << " " << x_min << " " << x_max << " " << A.geom->rank << std::endl;
  std::cout << "Local implementation at first (no mpi) "<< npx << " " << ipx << " " << A.geom->rank << std::endl;
...
//  local_int_t nlx = nx; nly = ny;local_int_t nlz = nz; //delete this
  ...
  std::cout << nx << " "<< ny << " "<< nz << " "<< nx*ny*nz << " " << nex*ney*nez << " "<< A.localNumberOfColumns << " "<< std::endl;
