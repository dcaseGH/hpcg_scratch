...
int ComputeSYMGS_ref( const SparseMatrix & A, const Vector & r, Vector & x) {
  ...
  std::cout << "ComputeSYMGS" <<std::endl;
...
+xv[(ix+1)+(iy+1)*nx+(iz+1)*ny*nx])/26.0;}
...
+xv[ix+1 + (iy+1)*nx + (iz+0)*nx*ny]
+xv[ix+1 + (iy+1)*nx + (iz+1)*nx*ny];
xv[ix+iy*nx+iz*ny*nx] = xv[ix+iy*nx+iz*ny*nx]/26.;
                }
             //std::cout << "rv xv i " << rv[ix+iy*nx+iz*ny*nx] << " "<< xv[ix+iy*nx+iz*ny*nx] << " " << ix+iy*nx+iz*ny*nx << std::endl;
             }
        }
}
/*
